  ...
  <div class="clear-float page-box-info">
    ...
    <div class="content-page">
      ...
      <div class="my-el-page">
        <el-pagination @current-change="handleCurrentChange" :current-page.sync="zjCurrentPage"
          layout="prev, pager, next" :total="zjTotalCount" :page-size="zjPageSize" prev-text='上一页' next-text='下一页'
          :background='true' :small='true' :pager-count="5">
        </el-pagination>
      </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      queryConditions(newVal, oldVal) {
        this.zjQueryConditions = newVal
      }
    },
    mounted() {
      let that = this
      this.token = this.$token
      Bus.$on('renewtable2', function(val) {
        that.zjCurrentPage = 1;
      })
      this.resetVal();
    },
    methods: {
      ...
      selectRepairsInfoPage() {
        let obj = {
          token: this.token,
          data: JSON.stringify(this.zjQueryConditions)
        };
        Bus.$emit("page-loading-" + this.id, true);
        this.$api
          .getMethod(this.hostName, this.methodsName, obj, this.token)
          .then(res => {
            setTimeout((load) => {
              Bus.$emit("page-loading-" + this.id, false);
            }, 500);

            if (res.appCode == 0) {
              let name = "page-" + this.id;
              this.zjTotalPage = res.resultList.totalPage;
              this.zjTotalCount = res.resultList.totalCount || res.resultList.total;
              Bus.$emit(name, res.resultList.result || res.resultList.rows);
            } else {
              Common.printErrorLog(this.hostName, this.methodsName);
            }
          })
          .catch(err => {
            setTimeout((load) => {
