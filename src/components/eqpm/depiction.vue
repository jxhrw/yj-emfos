  ...
  <div class="depiction-info">
    ...
    <div class="content">
      ...
      <el-row class="content-row-select">
        ...
        <el-col :span="9">
          <label>联系人</label>
          <el-input v-model="contactPerson" placeholder="请输入" size='mini' class="content-select"></el-input>
          <div class="linkman-explain">选填，默认报修人/联系人为用户</div>
        </el-col>
      </el-row>
      <el-row class="content-row-select content-row-bottom">
        <el-col :span="9">
          <label>电话</label>
          <el-input v-model="contactPersonTel" placeholder="请输入" size='mini' class="content-select"></el-input>
        </el-col>
      </el-row>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      select_Repairsource(newVal, oldVal) {
        this.changeRepSource(newVal);
      },
      textarea(newVal, oldVal) {
        Bus.$emit("failureDescrible", newVal); // 传输故障描述
      },
      contactPerson(newVal, oldVal) {
        Bus.$emit("contactPerson", newVal); // 传输报修联系人
      },
      contactPersonTel(newVal, oldVal) {
        Bus.$emit("contactPersonTel", newVal); // 传输报修联系人电话
      },
        ...
        this.$api
          ...
          .then(res => {
            if (res.appCode == 0) {
              ...
              this.select_Repairsource = this.options_Repairsource && this.options_Repairsource.length > 0 ? this
                .options_Repairsource[0].dicCode : '';
            } else {
