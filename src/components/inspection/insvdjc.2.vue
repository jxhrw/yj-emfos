<template>
    <div class="ej-content">
        <Layout @repair="devRepair" @another="checkAnother" :isRepaired="isRepaired">
            <h2 slot="pageTitle">巡检-{{pageTypeName()}}</h2>
            <Search slot="search" @searchAsk="selectRepairsInfoPage" :devType="pageTypeCode()"></Search>
                ...
                <div v-show="activeTab=='second'" class="right-scroll">
                    <div class="box-datas">
                        <div class="show-charts">
                            <div class="chart-total" v-if="realLaneInfo.allFlow">
                                当日车流量 <span>{{realLaneInfo.allFlow || 0}}辆</span>
                                实时车流量 <span>{{realLaneInfo.realFlow || 0}}辆/5分钟</span>
                            </div>
                            <div class="chart-line">
                                <Empty v-if="lineOption.series.length==0" class="cr-content" content="暂无数据" style="position:relative;"></Empty>
                                <chart v-else class="cr-content" :options="lineOption" :auto-resize="true"></chart>
                            </div>
                        </div>
                        <el-scrollbar class="ej-content-scrollbar fx-height">
                            <ul class="datas-info">
                                <li class="datats-title">
                                    ...
                                    <span class="datats-col5">流量(每5分钟)</span>
                                    <span class="datats-col2">采集时间</span>
                                    <span class="datats-col4">记录时间</span>
                                </li>
                                <li class="datas-flex" v-for="(item, index) in datasList" :key="index" @click="chooseItem(index)">
                                    <span class="datats-col3">{{item.DEV_WAY_ID}}</span>
                                    <span class="datats-col5">{{item.FLOW}}</span>
                                    <span class="datats-col2">{{item.COLLECT_TIME|timeNoSecond}}</span>
                                    <span class="datats-col4">{{item.RECORD_TIME|timeNoSecond}}</span>
                                </li>
                            </ul>
                        </el-scrollbar>
                    </div>
            ...
            <template slot="rightBase">
                <el-scrollbar class="right-scroll ej-content-scrollbar base-scroll">
                    <ul class="box-base">
                        ...
                        <li>
                            <label>类型</label>
                            <span>{{currentInfo.aaa||'视频检测'}}</span>
                        </li>
                        <li>
                            <label>管理部门</label>
    ...
    export default {
            ...
            this.lineOption = {
                tooltip: {
                    ...
                    formatter: function(params) {
                        let arr = [];
                        if (params.length > 0) {
                            arr.push(params[0].axisValueLabel);
                        }
                        params.map(val => {
                            arr.push(val.marker + val.seriesName + '：' + val.value + '辆/5分钟');
                        });
                        return arr.join('<br/>');
                    }
                },
        ...
        methods: {
            ...
            getPageVehSearFuc(id) {
                let hostOther = this.$config.collectionms_HOST;
                let method = '/collectionms-rest/getRealTimeData';
                let queryConditions = {
                    devId: id,
                    token: this.token
                };
                this.datasList = [];
                this.datasInfo = {};
                this.$api.postMethod(hostOther, method, qs.stringify(queryConditions), this.token)
                    .then(res => {
                        if (res.appCode == 0) {
                            ...
                            Common.printErrorLog(hostOther, method);
                        }
                    })
                    .catch(err => {
                        console.log(err);
                ...
                let host = this.$config.efoms_HOST;
                let method2 = this.$config.getDevWayRealFlow_GET;
                this.$api.getMethod(host, method2, { devId: id }, this.token)
                    .then(res => {
                        if (res.appCode == 0) {
                            let myRes = res.resultList;
                            this.realLaneInfo = myRes || {};
                            Object.keys(myRes).forEach(key => {
                                let list = myRes[key];
                                // eslint-disable-next-line
                                if (typeof(list) === "object") {
                                    let name = '车道' + list[0].devWayId;
                                    let date = [];
                                    let data = [];
                                    list.forEach(row => {
                                        let x = row.recordTime;
                                        x = x.substring(11, 16);
                                        let y = parseInt(row.flow);
                                        date.push(x);
                                        data.push(y);
                                    });
                                    let obj = {
                                    ...
                                    this.lineOption.xAxis.data = date;
                                    this.lineOption.legend.data.push(name);
                                    this.lineOption.series.push(obj);
                                }
                            });
                        } else {
                            Common.printErrorLog(host, method2);
                        }
                    })
                    .catch(err => {
                        console.log(err);
