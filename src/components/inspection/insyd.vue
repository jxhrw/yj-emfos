<template>
    <div class="ej-content">
        <Layout @repair="devRepair" @another="checkAnother" :isRepaired="isRepaired">
            <h2 slot="pageTitle">巡检-诱导屏</h2>
            <Search slot="search" @searchAsk="selectRepairsInfoPage" devType="REPDEVTYPE08"></Search>
            ...
            <div class="result-stas" slot="resultScreen">
                ...
                <ul class="result-stas-btns">
                    ...
                    <li>
                        连接状态：
                        <span :class="{'active':searchFlag.indexOf('LinkStatusTrue')>-1}" @click="flagFuc('LinkStatusTrue','LinkStatus')">联机{{countInfo.LinkStatusTrueCount}}</span>
                        <span :class="{'active':searchFlag.indexOf('LinkStatusFalse')>-1}" @click="flagFuc('LinkStatusFalse','LinkStatus')">脱机{{countInfo.LinkStatusFalseCount}}</span>
                        <span :class="{'active':searchFlag.indexOf('LinkStatusOther')>-1}" @click="flagFuc('LinkStatusOther','LinkStatus')">未知{{countInfo.LinkStatusOtherCount}}</span>
                    </li>
                    <li>
                        报警数量：
            ...
            <div class="left-table" slot="leftTable">
                <el-table :highlight-current-row="false" :data="tableData" stripe border class="content-table" @current-change="currentSelect" v-loading="isTableLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.7)">
                    ...
                    <el-table-column prop="ledTypeName" label="屏类型">
                        <template slot-scope="scope">
                            <p>{{scope.row.ledTypeName || '-'}}</p>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="连接状态">
                        <template slot-scope="scope">
                            <p v-if="String(scope.row.linkStatusName)=='在线'" class="device-status icon-normal">联机</p>
                            <p v-else-if="String(scope.row.linkStatusName)=='离线'" class="device-status icon-abnormal">脱机</p>
                            <p v-else>-</p>
                        </template>
                    </el-table-column>
                    <el-table-column label="当前节目">
                        <template slot-scope="scope">
                            <p v-if="String(scope.row.isLedProgram)=='true'" class="device-status icon-normal" style="cursor:pointer;">有</p>
                            <p v-else-if="String(scope.row.isLedProgram)=='false'" class="device-status icon-abnormal">无</p>
                            <p v-else>-</p>
                        </template>
                    </el-table-column>
            ...
            <template slot="rightTabs">
                <el-tabs v-model="activeTab" class="right-tabs" type="card" @tab-click="tabHandleClick">
                    <el-tab-pane label="节目信息" name="second"></el-tab-pane>
                ...
                <div v-show="activeTab=='second'" class="right-scroll">
                    <div class="box-datas">
                        <div class="show-img-box" v-loading="imgLoading" element-loading-text="加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.7)">
                            <img class="show-img" :src="datasInfo.picPath" alt="图像信息" style="height:180px;" @click="imgLarge">
                            <div class="time" v-if="datasInfo.progBeginTime">
                                <p>开始时间：{{datasInfo.progBeginTime||'-'}}</p>
                                <p>结束时间：{{datasInfo.progEndTime||'-'}}</p>
                            </div>

                            <div style="width:0;height:0;overflow:hidden;">
                                <viewer :images="imgFiles">
                                    <img id="imgId" :src="datasInfo.picPath">
                                </viewer>
                            </div>
                        </div>
                        <el-scrollbar class="ej-content-scrollbar fx-height">
                            <ul class="datas-info">
                                <li :class="[datasIndex==index?'active':'']" v-for="(item, index) in datasList" :key="index" @click="chooseItem(index)">{{item.progName}}</li>
                            </ul>
                        </el-scrollbar>
                    </div>
                </div>
            ...
            <template slot="rightBase">
                <el-scrollbar class="right-scroll ej-content-scrollbar base-scroll">
                    <ul class="box-base">
                        ...
                        <li>
                            <label>屏类型</label>
                            <span>{{currentInfo.ledTypeName||'--'}}</span>
                        </li>
                        <li>
                            <label>管理部门</label>
...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            datasInfo(val) {
                // if (val.picPath && val.picPath.indexOf('data:image') == -1) {
                //     let pic = this.$config.eqImg_HOST + encodeURIComponent(val.picPath);
                //     if (pic.indexOf('tp://') > -1) {
                //         // ftp或http图片转成base64的图片
                //         this.imgLoading = true;
                //         Common.baseImg(pic, this, this.$config.baseimgs).then(res => {
                //             val.picPath = res || '';
                ...
                // if (val.picPath && val.picPath.indexOf(this.$config.baseimgs) == -1) {
                //     let abc = this.$config.eqImg_HOST + encodeURIComponent(val.picPath);
                //     val.picPath = `${this.$config.baseimgs}?path=${abc}&token=${this.token}`;
                // }
                if (val.picPath && val.picPath.indexOf(this.$config.eqImg_HOST) == -1) {
                    val.picPath = this.$config.eqImg_HOST + encodeURIComponent(val.picPath);
                }
                newImg.src = val.picPath;
                this.imgLoading = true;
        ...
        methods: {
            ...
            getPageVehSearFuc(id) {
                let host = this.$config.efoms_HOST;
                let method = this.$config.getOnProgList_GET;
                let queryConditions = {
                    ledId: this.currentInfo.devId
                };
                this.datasList = [];
                this.datasInfo = {};
            ...
            imgLarge() {
                this.imgFiles = [];
                if (this.datasInfo.picPath) {
                    this.imgFiles.push(this.datasInfo.picPath);
                    setTimeout(() => {
