    ...
    <div class="ej-content">
        <Layout @repair="devRepair" @another="checkAnother" :isRepaired="isRepaired">
            <template slot="pageTitle">
                <h2>巡检-视频</h2>
                <div>
                    <div class="ej-content-goto" @click="gotoVdmd">
                        <p>视频模式</p>
                    </div>
                </div>
            </template>
            <Search slot="search" @searchAsk="selectRepairsInfoPage" cond="drainage" devType="REPDEVTYPE02"></Search>
            ...
            <div class="result-stas" slot="resultScreen">
                ...
                <ul class="result-stas-btns">
                    ...
                    <li>
                        有无图像：
                        <span :class="{'active':searchFlag.indexOf('isHavePicTrue')>-1}" @click="flagFuc('isHavePicTrue','isHavePic')">有{{countInfo.picZC}}</span>
                        <span :class="{'active':searchFlag.indexOf('isHavePicFalse')>-1}" @click="flagFuc('isHavePicFalse','isHavePic')">无{{countInfo.picYC}}</span>
                        <span :class="{'active':searchFlag.indexOf('isHavePicOther')>-1}" @click="flagFuc('isHavePicOther','isHavePic')">未知{{countInfo.picOther}}</span>
                    </li>
                    <li>
                        图像质量：
                        <span :class="{'active':searchFlag.indexOf('PicMassTrue','PicMass')>-1}" @click="flagFuc('PicMassTrue')">好{{countInfo.PicMassTrueCount}}</span>
                        <span :class="{'active':searchFlag.indexOf('PicMassFalse','PicMass')>-1}" @click="flagFuc('PicMassFalse')">坏{{countInfo.PicMassFalseCount}}</span>
                        <span :class="{'active':searchFlag.indexOf('PicMassOther','PicMass')>-1}" @click="flagFuc('PicMassOther')">未知{{countInfo.PicMassOtherCount}}</span>
                    </li>
                    <li>
                        报警数量：
            ...
            <div class="left-table" slot="leftTable">
                <el-table :highlight-current-row="false" :data="tableData" stripe border class="content-table" @current-change="currentSelect" v-loading="isTableLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.7)">
                    ...
                    <el-table-column prop="" label="有无图像" show-overflow-tooltip>
                        <template slot-scope="scope">
                            <p v-if="scope.row.videoCapUrl && ''!=scope.row.videoCapUrl && '-'!=scope.row.videoCapUrl" class="device-status icon-normal">有</p>
                            <p v-else-if="scope.row.videoCapUrl!=null" class="device-status icon-abnormal">无</p>
                            <p v-else>-</p>
                        </template>
                    </el-table-column>
                    <el-table-column prop="" label="图像质量" show-overflow-tooltip>
                        <template slot-scope="scope">
                            <template v-if="scope.row.videoCapUrl && ''!=scope.row.videoCapUrl && '-'!=scope.row.videoCapUrl">
                                <p v-if="String(scope.row.picStatusFlag)=='true'" class="device-status icon-normal">好</p>
                                <p v-else-if="String(scope.row.picStatusFlag)=='false'" class="device-status icon-abnormal">坏</p>
                                <p v-else>未知</p>
                            </template>
                            <template v-else>
                                <p>-</p>
            ...
            <template slot="rightTabs">
                <el-tabs v-model="activeTab" class="right-tabs" type="card" @tab-click="tabHandleClick">
                    ...
                    <el-tab-pane label="自动截图" name="second"></el-tab-pane>
                ...
                <div v-show="activeTab=='second'" class="right-scroll">
                    <div class="box-datas">
                        <div class="show-img-box" v-loading="imgLoading" element-loading-text="加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.7)">
                            <img class="show-img" :src="currentInfo.videoCapUrl" alt="图像信息" @click="imgLarge">

                            <div style="width:0;height:0;overflow:hidden;">
                                <viewer :images="imgFiles">
                                    <img id="imgId" :src="currentInfo.videoCapUrl">
                                </viewer>
                            </div>
                        </div>
                        <el-scrollbar class="ej-content-scrollbar fx-height">
                            <div class="time-list">
                                ...
                                <div class="progress-bar">
                                    <ul class="pgss">
                                        <li v-for="(item,index) in datasList" :key="index" :style="{'width':item.width+'%','left':item.left+'%'}"></li>
                                    </ul>
                                    <ul class="scale">
                                        <li style="left:0;">0</li>
                                        <li style="left:25%;">6</li>
                                        <li style="left:50%;">12</li>
                                        <li style="left:75%;">18</li>
                                        <li style="left:100%;">24</li>
                                    </ul>
                                </div>
                            </div>
                        </el-scrollbar>
            ...
            <template slot="rightBase">
                <el-scrollbar class="right-scroll ej-content-scrollbar base-scroll">
                    <ul class="box-base">
                        ...
                        <li>
                            <label>视频类型</label>
                            <span>{{currentInfo.videoDevTypeName}}</span>
                        </li>
                        <li>
                            <label>管理部门</label>
    ...
    export default {
        ...
        watch: {
            ...
            currentInfo(val) {
                if (val) {
                    ...
                    // if (val.videoCapUrl && val.videoCapUrl.indexOf('tp://') > -1) {
                    //     // ftp或http图片转成base64的图片
                    //     this.imgLoading = true;
                    //     Common.baseImg(val.videoCapUrl, this, this.$config.baseimgs).then(res => {
                    //         val.videoCapUrl = res || '';
                    ...
                    if (val.videoCapUrl && val.videoCapUrl.indexOf(this.$config.baseimgs) == -1 && '' != val.videoCapUrl && '-' != val.videoCapUrl) {
                        val.videoCapUrl = `${this.$config.baseimgs}?token=${this.token}&path=${encodeURIComponent(endval.videoCapUrl)}`;
                    }
                    newImg.src = val.videoCapUrl;
                    this.imgLoading = true;
        ...
        methods: {
            ...
            getPageVehSearFuc(id) {
                let host = this.$config.efoms_HOST;
                let method = '/efoms-rest/webService/getPlaybackList';
                let queryConditions = {
                    videoid: this.currentInfo.videoPlayId,
                ...
                this.$api.getMethod(host, method, queryConditions, this.token)
                    .then(res => {
                        let recordinfo = JSON.parse(res.resultList.recordinfo) || {};
                        console.log(recordinfo);
                        let toDay = Common.dateFormat('yyyy-MM-dd', new Date()) + ' 00:00:00';
                        Object.keys(recordinfo).forEach(key => {
                            // if (key.indexOf('file_path') > -1) {
                            //     let arr = recordinfo[key].split('-');
                            //     if (arr.length > 1) {
                            //         let left = (arr[0] - new Date(toDay) / 1000) / 60; //开始时间的秒数
                            //         let width = (arr[1] - arr[0]) / 60;
                            //         let secondall = 1440; //一天的秒数
                            //         let obj = {
                            //             left: (left > 0 ? left : 0) * 100 / secondall,
                            //             width: (width > 0 ? width : 0) * 100 / secondall
                            //         };
                            //         this.datasList.push(obj);
                            //     }
                            // }

                            if (key.indexOf('s_time') > -1) {
                                let num = key.split('s_time')[1];
                                if (recordinfo[`e_time${num}`]) {
                                    let arr = [recordinfo[`s_time${num}`], recordinfo[`e_time${num}`]];
                                    let left = (arr[0] - new Date(toDay) / 1000) / 60; //开始时间的秒数
                                    let width = (arr[1] - arr[0]) / 60;
                                    let secondall = 1440; //一天的秒数
                                    let obj = {
            ...
            getVideoList() {
                this.videoList = [{
                    manageId: this.currentInfo.videoPlayId,
                    videoName: this.currentInfo.devName,
                }];
                this.videoIndex = 0;
            ...
            imgLarge() {
                this.imgFiles = [];
                if (this.currentInfo.videoCapUrl) {
                    this.imgFiles.push(this.currentInfo.videoCapUrl);
                    setTimeout(() => {
            ...
            gotoVdmd() {
                let url = this.$config.ProjectName + "/insvdmd.html?token=" + this.token;
