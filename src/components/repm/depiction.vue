...
<div class="depiction-info">
    ...
    <div class="content">
        <el-table highlight-current-row :data="tableData" border @current-change='currentSelect' class="content-table" v-loading="isTableLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.7)">
          ...
          <el-table-column prop="devName" label="设备名称" show-overflow-tooltip  min-width="120"></el-table-column>
          ...
          <el-table-column prop="devDeptName" label="管理单位" show-overflow-tooltip v-if="JSON.stringify(multipleSelection).indexOf('管理单位')>-1"></el-table-column>
          <el-table-column prop="failureTypeName" label="故障类型" show-overflow-tooltip></el-table-column>
          <el-table-column prop="repDeptName" label="报修单位" show-overflow-tooltip v-if="JSON.stringify(multipleSelection).indexOf('报修单位')>-1"></el-table-column>
          ...
          <el-table-column prop="repStatusName" label="报修状态" width=75 show-overflow-tooltip></el-table-column>
          <el-table-column prop="pressTimes" label="催办次数" width=75 show-overflow-tooltip v-if="JSON.stringify(multipleSelection).indexOf('催办次数')>-1"></el-table-column>
...
export default {
  data() {
    return {
      ...
      operExplain4Cancel: '',
      selectRepairsStatus: '',
      selectRepairsStatusName: '',
      typeTableData: [{"name": "所属辖区"}, {"name": "管理单位"}, {"name": "报修单位"}, {"name": "报修人"}, {"name": "催办次数"}],
  ...
  methods: {
    ...
    showRevoke() {
      ...
      // if (this.selectRepairsStatus == 'REPAIRSTATUS04' || this.selectRepairsStatus == 'REPAIRSTATUS03') {
      //   return this.$message.error('该数据已撤销或已拒绝');
      // }
      if (this.selectRepairsStatus != 'REPAIRSTATUS01') {
        let tips = "该数据" + (this.selectRepairsStatusName || '不是待审核') + "不能撤销";
    ...
    showUrge() {
      ...
      if (this.selectRepairsStatus != 'REPAIRSTATUS01') {
        let tips = "该数据" + (this.selectRepairsStatusName || '不是待审核') + "不能催办";
        return this.$message.error(tips);
      }
    ...
    currentSelect(e) {
      ...
      this.selectRepairsStatus = e.repStatusCode;
      this.selectRepairsStatusName = e.repStatusName;
    ...
    dataDetail(item) {
      ...
      .then(res => {
        if (res.appCode == 0) {
          ...
          window.location.href = this.$config.ProjectName + '/repdesc.html?token=' + this.token;
        } else {
