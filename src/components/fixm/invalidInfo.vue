  ...
  <div>
    <div class="base-info">
      <div class="tabs">
        ...
        <div class="other-tabs" @click="changeNum(2)">
          <span>维修工单</span>
          <!--             <span>待确认（99）</span> -->
        </div>
        <div class="other-tabs active" @click="changeNum(3)">
          <span>无效工单</span>
        </div>
      ...
      <div class="content">
        <el-row class="content-row-select">
          <el-col :span="7">
            <label>关键字</label>
            <el-input v-model="queryConditions.key" placeholder="设备名称/设备编号" size='mini' class="content-select"
              clearable></el-input>
          </el-col>
          <el-col :span="7">
            <label>报修时间</label>
        ...
        <el-row v-show="conditionVisible" class="content-row-select">
          <el-col :span="7">
            <label>工单编号</label>
            <el-input v-model="queryConditions.workordersId" placeholder="请输入" size='mini' class="content-select"
              clearable></el-input>
          </el-col>
          <el-col :span="7">
            <label>所属辖区</label>
            ...
            <el-input v-model="devAreaName" placeholder="请选择" size='mini' class="content-select" clearable readonly
              v-popover:popoverSelectRegion></el-input>
            <el-popover highlight-current popper-class="region-popper" ref="popoverSelectRegion"
              placement="bottom-start" trigger="click" v-model="selectRegionVisible">
              <el-scrollbar class="region-scroll">
    ...
    <div class="depiction-info">
      <div class="title">
        ...
        <el-popover ref="popoverMultiple" placement="left" title="选择可见内容" popper-class="table-list-popver"
          trigger="click" @after-enter="initMultiple">
      ...
      <div class="content">
        <el-table highlight-current-row :data="tableData" border @current-change='currentSelect' class="content-table"
          ...
          <el-table-column prop="workordersId" label="工单编号" show-overflow-tooltip
            v-if="JSON.stringify(multipleSelection).indexOf('工单编号')>-1"></el-table-column>
          ...
          <el-table-column prop="devAreaName" label="所属辖区" show-overflow-tooltip
            v-if="JSON.stringify(multipleSelection).indexOf('所属辖区')>-1"></el-table-column>
          <el-table-column prop="devDeptName" label="管理部门" show-overflow-tooltip
            v-if="JSON.stringify(multipleSelection).indexOf('管理部门')>-1"></el-table-column>
          ...
          <el-table-column prop="finishTime" label="结束时间" show-overflow-tooltip
            v-if="JSON.stringify(multipleSelection).indexOf('结束时间')>-1"></el-table-column>
          <el-table-column prop="failureTypeName" label="无效原因" show-overflow-tooltip></el-table-column>
          <el-table-column prop="repPerson" label="操作人" show-overflow-tooltip
            v-if="JSON.stringify(multipleSelection).indexOf('操作人')>-1"></el-table-column>
        ...
        <div class="content-page">
          ...
          <div class="my-el-page">
            <el-pagination @current-change="handleCurrentChange" :current-page.sync="queryConditions.currentPage"
              layout="prev, pager, next" :total="totalCount" :page-size="queryConditions.pageSize" prev-text='上一页'
      ...
      <el-dialog title="报修催办" :visible.sync="dialogUrgeVisible" width='400px' class="dialog-urge">
        <div class="dialog-main">
          ...
          <textarea rows="8" cols="40" placeholder="请输入" class="dialog-content"></textarea>
        </div>
        <div slot="footer" class="dialog-footer">
          <el-button type="primary" @click="dialogUrgeVisible = false" size='mini' class="submit">提 交</el-button>
      ...
      <el-dialog title="撤销申请" :visible.sync="dialogRevokeVisible" width='400px' class="dialog-urge">
        <div class="dialog-main">
          <div class="revoke-reason">
            ...
            <el-select v-model="value" placeholder="请选择" size='mini' class="content-select">
              <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </div>
        ...
        <div slot="footer" class="dialog-footer">
          <el-button type="primary" @click="dialogRevokeVisible = false" size='mini' class="submit">提 交</el-button>
  ...
  export default {
    data() {
      return {
        ...
        value: "",
        queryConditions: {
          workordersStatusCode: "ORDERSSTATUS08",
          workordersId: "",
          key: "",
          devAreaCode: "",
          devDeptId: "",
    ...
    methods: {
      exportExcel() {
        ...
        let method = this.$config.exportWorkorders_GET;
        if (this.repairDate) {
          this.queryConditions.startTime = Common.dateFormat('yyyy-MM-dd hh:mm:ss', this.repairDate[0]);
          this.queryConditions.endTime = Common.dateFormat('yyyy-MM-dd hh:mm:ss', this.repairDate[1]);
        } else {
        ...
        this.$api
          ...
          .then(res => {
            if (res.appCode == 0) {
              ...
              var type = item.workordersStatusCode ? item.workordersStatusCode : "";
              switch (type) {
                case "ORDERSSTATUS01":
                  window.location.href = this.$config.ProjectName +
                    "/create.html?type=fixm&token=" + this.token;
                  break;
                default:
                  window.location.href = this.$config.ProjectName +
                    "/fixdesc.html?type=dispatch&token=" + this.token;
                  break;
              }
            } else {
