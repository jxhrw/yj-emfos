...
<div>
  <div class="base-info">
    <div class="tabs">
        ...
        <div class="other-tabs active" @click="changeNum(1)">
            <span class="tabs-name">报修审核</span>
            <span class="tabs-count">待审核({{serverPUshData.REP_MSG_APPLY}})</span>
        </div>
...
<!--             <span>待确认（99）</span> -->
    ...
    <div class="content">
        <el-row class="content-row-select">
            <div class="quick-query" :class="{'active':lableClick==='1'}" @click="changeLableClick('1')">
                <label for="">催办提醒</label>
                <span>{{serverPUshData.REP_MSG_PRESS}}</span>
            </div>
        </el-row>
        <el-row class="content-row-select">
            <el-col :span="7">
            ...
            <el-col :span="7">
                <label>报修状态</label>
                <el-select v-model="queryConditions.repStatusCode" placeholder="请选择" size='mini' class="content-select" clearable>
        ...
        <el-row v-show="conditionVisible" class="content-row-select">
            ...
            <el-col :span="7">
                <label>报修单位</label>
                <!-- <el-select v-model="queryConditions.repairsId" placeholder="请选择" size='mini' class="content-select">
...
<div class="depiction-info">
    ...
    <div class="content">
        <el-table highlight-current-row :data="tableData" border @current-change='currentSelect' class="content-table" v-loading="isTableLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.7)">
          ...
          <el-table-column prop="repDeptName" label="报修单位" show-overflow-tooltip v-if="JSON.stringify(multipleSelection).indexOf('故障类型')>-1"></el-table-column>
          <el-table-column prop="repPersonName" label="报修人" show-overflow-tooltip v-if="JSON.stringify(multipleSelection).indexOf('报修人')>-1"></el-table-column>
          <el-table-column prop="repDate" label="报修时间" show-overflow-tooltip></el-table-column>
          <el-table-column prop="repStatusName" label="报修状态" show-overflow-tooltip></el-table-column>
          <el-table-column prop="pressTimes" label="催办次数"></el-table-column>
...
export default {
  data() {
    return {
      ...
      typeTableData: [{"name": "所属辖区"}, {"name": "管理部门"}, {"name": "故障类型"}, {"name": "报修人"}],
  ...
  watch: {
    ...
    selectDept(newVal, oldVal) {
        if (newVal === '') {
            this.queryConditions.repairsId = '';
            setTimeout(() => {
  ...
  mounted() {
    ...
    this.getDicInfo('REPAIRSTATUS').then(res => {
        if (res.appCode == 0) { this.repairStatusList = res.resultList; }
    });
  ...
  methods: {
    ...
    changeLableClick(value) {
      ...
      this.queryConditions.press = this.lableClick;
      this.searchTable();
    },
    ...
    handleNodeClickDept(data, node, e) {
        ...
        if (data) {
            ...
            this.queryConditions.repairsId = data.id;
            this.selectDeptVisible = false;
        }
    },
    serverPUsh() {
    ...
    repairsOperationCount() {
      var url =
        this.$config.efoms_HOST + this.$config.repairsOperationCount_GET;
      var header = {
      ...
      this.$api
        ...
        .then(res => {
          if (res.appCode == 0) {
            ...
            Common.printErrorLog(this.$config.efoms_HOST, this.$config.repairsOperationCount_GET);
          }
        })
        .catch(err => {
          Common.printErrorLog(this.$config.efoms_HOST, this.$config.repairsOperationCount_GET);
    ...
    selectRepairsInfoPage() {
      ...
      var url =
        this.$config.efoms_HOST + this.$config.selectRepairsInfoPage_GET;
      var header = {
        "Content-Type": "application/x-www-form-urlencoded",
        token: this.token
      };
    ...
    dataDetail(item) {
      var url =
        this.$config.efoms_HOST + this.$config.getRepairsInfoById_GET;
      var header = {
        "Content-Type": "application/x-www-form-urlencoded",
        token: this.token
      };
      ...
      .then(res => {
        if (res.appCode == 0) {
          ...
          window.location.href = this.$config.ProjectName + '/repdesc.html?type=fixm&token=' + this.token;
        } else {
...
.dialog-urge .dialog-main .dialog-content{background: #F9FAFC;border: 1px solid #E1E7ED;}
...
.region-popper{min-width: 220px;padding: 5px 0;}
.region-scroll {height: 246px;}
.region-scroll .el-scrollbar__wrap{overflow-x: hidden;}
</style>
