...
<div class="depiction-info">
    ...
    <div class="content">
        <el-row class="content-row-select" v-if='isShow_Repdevtype'>
            <el-col :span="9">
                <label>故障类型</label>
                <el-select v-model="repdevtype" placeholder="请选择" size='mini' class="content-select" @change='changeFailureType'>
                    <el-option v-for="item in repdevtypeList" :key="item.dicCode" :label="item.dicName" :value="item.dicCode">
                    </el-option>
                </el-select>
            </el-col>
        </el-row>
        <el-row class="content-row-select">
            <el-col :span="9">
                ...
                <el-input type="textarea" :rows="3" placeholder="请输入内容" class="content-textarea" v-model="operExplain" resize="none">
                </el-input>
            </el-col>
        </el-row>
...
export default {
  ...
  watch: {
    operExplain(newVal, oldVal) {
      Bus.$emit("fackback_operExplain", newVal); // 传输故障描述
    },
    ...
    fileInfoList(newVal, oldVal) {
      Bus.$emit("fackback_fileInfoList", newVal); // 传输文件合集
    }
  },
  methods: {
    changeFailureType(val) {
      this.repdevtypeList.forEach(e => {
        if (e.dicCode == val) {
          Bus.$emit("fackback_failureType", e); // 传输故障类型
        }
      });
    },
    enter(index) {
    ...
    getRepdevtype(selectRepdevcategory) {
      ...
      this.$api
        ...
        .then(res => {
          if (res.appCode == 0) {
            this.repdevtypeList = res.resultList;
          } else {
            Common.printErrorLog(this.$config.ubms_HOST, this.$config.getDeviceDic_GET);
          }
        })
