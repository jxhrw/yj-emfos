...
<div class="depiction-info">
    ...
    <div class="content">
        <el-row class="content-row-select" v-if='isShow_Repdevtype'>
            <el-col :span="9">
                <label>故障类型</label>
                <el-select v-model="select_Repdevtype" placeholder="请选择" size='mini' class="content-select" @change='changeFailureType'>
                    <el-option v-for="item in options_Repdevtype" :key="item.dicCode" :label="item.dicName" :value="item.dicCode">
                    </el-option>
                </el-select>
            </el-col>
        </el-row>
        <el-row class="content-row-select">
            <el-col :span="9">
                ...
                <el-input type="textarea" :rows="3" placeholder="请输入内容" class="content-textarea" v-model="textarea" resize="none">
                </el-input>
            </el-col>
        </el-row>
            ...
            <el-col :span="24" class="content-row-img">
                ...
                <template v-for="(item,index) in imgFiles">
                  <div class="img-preview" @mouseenter="enter(index)" @mouseleave="leave()" :key="index">
                    ...
                    <div class="img-del" v-show="seen&&index==current" @click="delImg(index)"><p>删除</p></div>
                  </div>
                </template>
        ...
        <el-row class="content-row-select">
            <el-col :span="9">
                <label>报修来源</label>
                <el-select v-model="select_Repairsource" placeholder="请选择" size='mini' class="content-select" @change='changeRepSource'>
                    <el-option v-for="item in options_Repairsource" :key="item.dicCode" :label="item.dicName" :value="item.dicCode">
                    </el-option>
                </el-select>
            </el-col>
            <el-col :span="9">
                <label>报修人员</label>
                <el-select v-model="repPersonId" placeholder="请选择" size='mini' class="content-select"  @change='changePersonList'>
...
export default {
  ...
  mounted() {
    ...
    Bus.$on("select_Devicecategory", value => {
      ...
      Bus.$emit("failureType", { failureTypeCode: "", failureTypeName: "" }); // 传输故障类型
      if (value == "REPDEVCATEGORY01") {
        this.isShow_Repdevtype = true;
      } else if (value == "REPDEVCATEGORY02") {
        this.isShow_Repdevtype = false;
      }
    });
...
<style scoped>
.depiction-info{margin: 14px 14px 0px 14px;background: #FFFFFF;border: 1px solid #E4EAEE;border-radius: 2px;}
...
.depiction-info .content{padding-top: 5px;padding-left: 40px;}
...
.depiction-info .content .file-preview{display: inline-block;margin-right: 20px;}
