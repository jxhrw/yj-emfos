    ...
    <div class="depiction-info">
        ...
        <div class="content">
            <el-table highlight-current-row :data="tableData" stripe border class="content-table" @current-change='currentSelect' v-loading="isTableLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.7)">
                ...
                <el-table-column prop="devCategoryName" label="设备类别" show-overflow-tooltip v-if="JSON.stringify(multipleSelection).indexOf('设备类别')>-1">
                    <template slot-scope="scope">
                        <p>{{scope.row.devCategoryName || '-'}}</p>
                    </template>
                </el-table-column>
                <el-table-column prop="devTypeName" label="设备类型" show-overflow-tooltip v-if="JSON.stringify(multipleSelection).indexOf('设备类型')>-1">
                    <template slot-scope="scope">
            ...
            <EJ-Paging ref="tableList" id="journal" :totalCount="totalCount" :totalPage="totalPage" :queryConditions="queryConditions" :hostName="pageHost" :methodsName="pageMethods"></EJ-Paging>
        </div>
