                    ...
                    <div class="depiction-info">
                        ...
                        <div class="content">
                            <el-table :highlight-current-row='false' :data="tableData" stripe border class="content-table" @current-change="currentSelect" v-loading="isTableLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.7)">
                                ...
                                <el-table-column label="卡口名称" show-overflow-tooltip min-width="100">
                                    <template slot-scope="scope">
                                        {{scope.row.BAYONETNAME || scope.row.BAYONET_ID}}
                                    </template>
                                </el-table-column>
                                <el-table-column prop="PASS_TIME" label="最新过车时间" show-overflow-tooltip min-width="100"></el-table-column>
                                <el-table-column prop="RECEIVE_TIME" label="最新接收时间" show-overflow-tooltip min-width="100"></el-table-column>
                                <el-table-column prop="LATE_TIME_TOTAL" label="接收耗时(秒)" show-overflow-tooltip min-width="100">
                                    <template slot-scope="scope">
                                        {{scope.row.RECEIVE_TIME|timeDisparity(scope.row.PASS_TIME)}}
                                    </template>
                                </el-table-column>
                                <el-table-column prop="RECEIVE_NUM" label="今日接收量" show-overflow-tooltip min-width="90"></el-table-column>
                                <el-table-column v-if="showPerc" prop="RECEIVE_NUM_DAY" label="昨日同期接收量" show-overflow-tooltip min-width="110"></el-table-column>
                                <el-table-column v-if="showPerc" prop="RECEIVE_NUM_DAY_RATE" label="环比" show-overflow-tooltip min-width="50"></el-table-column>
                                <el-table-column v-if="showPerc" prop="RECEIVE_NUM_WEEK" label="上周同期接收量" show-overflow-tooltip min-width="110"></el-table-column>
                                <el-table-column v-if="showPerc" prop="RECEIVE_NUM_WEEK_RATE" label="同比(秒)" show-overflow-tooltip></el-table-column>
                                <el-table-column prop="LATE_NUM" label="今日延时量" show-overflow-tooltip min-width="100"></el-table-column>
                                <el-table-column prop="LATE_PERCENT" label="延时数据占比" show-overflow-tooltip min-width="100"></el-table-column>
                                <el-table-column prop="LATE_TIME_PERCENT" label="平均延时(秒)" show-overflow-tooltip min-width="100"></el-table-column>
                                <el-table-column v-if="showFurure" prop="FUTURE_NUM" label="今日超前量" show-overflow-tooltip min-width="90"></el-table-column>
                                <el-table-column v-if="showFurure" prop="FUTURE_PERCENT" label="超前数量占比" show-overflow-tooltip min-width="100"></el-table-column>
                                <el-table-column v-if="showFurure" prop="FUTURE_TIME_PERCENT" label="平均超前时间(秒)" show-overflow-tooltip min-width="120"></el-table-column>
                            </el-table>
    ...
    export default {
        ...
        mounted() {
            ...
            switch (this.query.type) {
                case "EFFECTIVE_NUM":
                    this.queryShow.typeShow = "有数据卡口数";
                    break;
                case "INEFFECTIVE_NUM":
                    this.queryShow.typeShow = "无数据卡口数";
                    break;
                case "REAL_LESS":
                    this.queryShow.typeShow = "数据实时性：3小时以内";
                    break;
                case "REAL_MIDDLE":
                    this.queryShow.typeShow = "数据实时性：超过3小时";
                    break;
                case "REAL_MORE":
                    this.queryShow.typeShow = "数据实时性：超过半天";
                    break;
                case "RECEIVE_LESS":
                    this.queryShow.typeShow = "今日接收数据量：100以内";
                    break;
                case "RECEIVE_MIDDLE":
                    this.queryShow.typeShow = "今日接收数据量：超过100";
                    break;
                case "RECEIVE_MORE":
                    this.queryShow.typeShow = "今日接收数据量：超过5000";
                    break;
                case "LATE_LESS":
                    this.queryShow.typeShow = "今日延时情况：30秒以内";
                    break;
                case "LATE_MIDDLE":
                    this.queryShow.typeShow = "今日延时情况：超过30秒";
                    break;
                case "LATE_MORE":
                    this.queryShow.typeShow = "今日延时情况：超过3分钟";
                    break;
                case "FURURE_LESS":
                    this.queryShow.typeShow = "今日超前情况：30秒以内";
                    break;
                case "FURURE_MIDDLE":
                    this.queryShow.typeShow = "今日超前情况：超过30秒";
                    break;
                case "FURURE_MORE":
                    this.queryShow.typeShow = "今日超前情况：超过3分钟";
                    break;
                default:
                    break;
            };
        ...
        methods: {
            ...
            searchTable() {
                ...
                this.$api.get(
                        `${this.$config.idcs_HOST}/vehPass/pageMonitorBayonetReceive4Efoms.htm`, { token: this.token, data: JSON.stringify(this.queryConditions) },
                        null
                    )
                    .then(res => {
