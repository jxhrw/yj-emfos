    ...
    <div class="operation-main" v-show="isShow">
      ...
      <div class="operation-content">
        <div class="complete-content" v-show="isActive!=3">
          <el-row class="content-row-select">
            <el-col :span="9">
              ...
              >
                <el-option
                  v-for="item in options_Repdevtype"
                  :key="item.dicCode"
                  :label="item.dicName"
                  :value="item.dicCode"
                ></el-option>
            ...
            <el-col :span="9" v-if="isActive!=2">
              <label>设备状态</label>
              <el-select
                v-model="select_DevStatus"
                placeholder="请选择"
                size="mini"
                class="content-select"
        ...
        <div class="complete-content" v-show="isActive==3">
          <el-row class="content-row-select">
            <el-col :span="24">
              <label class="content-label-fix"><span>*</span>申请完成时间</label>
                <el-date-picker
                  style="margin-left: 84px;width: 300px;"
          ...
          <el-row class="content-row-select">
            <el-col :span="24">
              <label class="content-label-fix"><span style="left: 17px;">*</span>延期原因</label>
              <el-input
                type="textarea"
                :rows="3"
                placeholder="请输入内容"
...
export default {
  ...
  watch: {
    isActive(newVal, oldVal) {
      this.isOver = newVal == 1 ? 1 : 0;
      let arr = [];
      if (this.workordersInfo.workordersRecordMap && this.workordersInfo.workordersRecordMap) {
        arr = this.workordersInfo.workordersRecordMap.fackbackList || [];
      }
      if (this.isActive != 3) {
        ...
        this.failureReason = '';
        this.operExplain = '';
        this.select_DevStatus = '';
        let key = this.isActive == 1 ? '完成反馈' : '进度反馈';
        arr.forEach(val => {
          if (val.operResult == key) {
            this.failureTypeCode = val.failureTypeCode;
            this.failureTypeName = val.failureTypeName;
            this.failureReason = val.failureReason;
            this.operExplain = val.operExplain;
            this.select_DevStatus = '';
          }
        });
      }
  ...
  methods: {
    ...
    postponeWorkorders() {
      ...
      var deadlineDate = Common.dateFormat('yyyy-MM-dd hh:mm:ss', this.deadlineDate);
      var url =
        this.$config.efoms_HOST +
...
.operation-info .operation-main .operation-title label{font-size: 14px;color: #157745;margin-left: 36px;margin-right: 111px;line-height: 43px;position: relative;}
...
.operation-info .operation-main .operation-content{height: 254px;border-top: 1px solid #80D1B3;margin-left: 37px;margin-right: 62px;}
.operation-info .operation-main .operation-content .complete-content{margin-top: 20px;}
.operation-info .operation-main .operation-content .complete-content .content-row-select{margin-top: 10px;}
...
.operation-info .operation-main .operation-content .complete-content label {color: #4f5a64;display: inline-block;width: 48px;font-size: 12px;margin-right: 12px;text-align: right;position: relative;}
.operation-info .operation-main .operation-content .complete-content span {font-size: 12px;color: #737e84;}
.operation-info .operation-main .operation-content .complete-content label span{color: red;position: absolute;top: 2px;left: -7px;}
...
.operation-info .operation-main .operation-content .complete-content .content-textarea-fix {width: 300px;margin-left: 84px;}
